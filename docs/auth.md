...
2. Popup sends an `AUTH_SIGN_IN` message to the background service worker (and may use `AUTH_REAUTHORIZE` for silent re-auth flows).
...
7. The popup reads the stored auth state from `browser.storage.local` (e.g. on load or via polling) and updates its UI to reflect the successful login.
