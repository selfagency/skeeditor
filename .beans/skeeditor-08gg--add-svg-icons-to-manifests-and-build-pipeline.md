...
Add skeeditor.svg (transparent) and skeeditor.svg (solid background) icons to the extension manifests using @resvg/resvg-js to generate PNGs at required sizes.
...
- Added `buildIcons()` function to `scripts/build.ts` using `@resvg/resvg-js` (resvg) to render `skeeditor.svg` (transparent) at 16, 32, 48, 128px and `skeeditor.svg` (solid) at 16, 32px to `dist/<browser>/icons/`.
