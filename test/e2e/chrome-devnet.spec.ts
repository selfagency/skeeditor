 ...
 */
import { test, expect } from './fixtures/chromium-devnet-extension';
 ...
 */
test(
  'content script injects Edit button for own real devnet post',
 ...
 */
test(
  'edit button is absent on other users posts',
  async ({ page, devnetSession, devnetPost, routeDevnetBskyApp }) => {
    await routeDevnetBskyApp();
    await page.goto(`https://bsky.app/profile/${devnetSession.did}/post/${devnetPost.rkey}`);
 ...
 */
test(
  'edit modal saves real post text to devnet PDS',
  async ({ page, devnetSession, devnetPost, routeDevnetBskyApp }) => {
    await routeDevnetBskyApp();
    await page.goto(`https://bsky.app/profile/${devnetSession.did}/post/${devnetPost.rkey}`);
 ...
 */
test(
  'unauthenticated user sees no Edit buttons on devnet post page',
 ...
 */
test(
  'concurrent edit conflict shows retry prompt',
  async ({ page, devnetSession, devnetPost, routeDevnetBskyApp }) => {
    await routeDevnetBskyApp();
    await page.goto(`https://bsky.app/profile/${devnetSession.did}/post/${devnetPost.rkey}`);
